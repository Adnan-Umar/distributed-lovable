package com.adnanumar.distributed_lovable.api_gateway.filter;

import com.adnanumar.distributed_lovable.api_gateway.config.SecurityProperties;
import com.adnanumar.distributed_lovable.api_gateway.error.ApiError;
import com.adnanumar.distributed_lovable.api_gateway.service.JwtGatewayService;
...
public class GatewayJwtAuthFilter implements GlobalFilter, Ordered {
    ...
    final JwtGatewayService jwtGatewayService;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // Bearer jsdgaiofg.kjsdhfiuh.lghiuhse
        String token = authHeader.substring(7);

        try {
            jwtGatewayService.validateToken(token);
            log.info("Jwt token for valid for path : {}", path);
        } catch (Exception e) {
